...
\Tree [ \qroof{João trabalha}.S$_{1}$ e \qroof{Maria
	estuda}.S$_{2}$ ].S
...
\Tree [ \qroof{João trabalha}.S$_{1}$ [ [ e ].Coord \qroof{Maria
estuda}.S$_{2}$ ].Coord$^{\prime}$ ].CoordP
...
\n \den{e} = $(\lambda p_{t}.\ \lambda q_{t}.\ p=1\ \text{e}\
q=1)$\\
...
\n \den{e} = $(\lambda p_{t}.\ \lambda q_{t}.\ p=1\ \&\
q=1)$\\ 
...
\n \den{Coord$^{\prime}$} = \den{Coord}(\den{S$_{2}$})\\
...
\n \den{S$_{2}$} = 1 \textit{sse} $\predica{estuda}{maria}$ \\

\n Temos que:\\

\n \den{Coord$^{\prime}$} = $\lambda q_{t}.\ \predica{estuda}{maria}\ \&\ q=1$\\

\n Aplica\-ção funcional é então novamente utilizada para derivar
as condi\-çõ\-es de verdade de CoordP:\\

\n \den{CoordP} = \den{Coord$^{\prime}$}(\den{S$_{1}$})\\
...
\n \den{S$_{1}$} = 1 \textit{sse} $\predica{trabalha}{joão}$\\

\n temos que\\

\n \den{CoordP} = 1 \textit{sse} $\predica{estuda}{maria}\ \&\ \predica{trabalha}{joão} $\\
...
\n \den{CoordP} = 1 \textit{sse} $\predica{trabalha}{joão}\ \&\  \predica{estuda}{maria}$\\
...
\n \den{e$'$} = $\lambda F_{\langle e,t\rangle}.\lambda G_{\langle e,t\rangle}.\lambda x_{e}.\
F(x)=1\ \&\ G(x)=1$\\
...
\n \den{Coord$'$} = \den{Coord}(\den{VP$_{2}$})\\
...
\n \den{VP$_{2}$} = $\lambda x.\ \predica{estuda}{x}$ \\

\n e que, portanto:\\

\n \den{VP$_{2}$}($x$) = 1 \textit{sse} $\predica{estuda}{x}$ \\

\n temos que:\\

\n \den{Coord$'$} = $\lambda G.\lambda x.\ \predica{estuda}{x}\
\&\ G'(x)$\\

\n Quanto a CoordP, valemo-nos de aplica\-ção funcional
novamente:\\

\n \den{CoordP} = \den{Coord$'$}(\den{VP$_{1}$})\\
...
\n \den{VP$_{1}$} = $\lambda x.\ \predica{trabalha}{x}$ \\

\n e que, portanto: \\

\n \den{VP$_{1}$}(\textit{x}) = 1 \textit{sse} $\predica{trabalha}{x}$\\

\n temos que:\\

\n \den{CoordP} = $\lambda x_{e}.\ \predica{estuda}{x}\ \&\ \predica{trabalha}{x}$\\
...
\n \den{CoordP} = $\lambda x_{e}.\ \predica{trabalha}{x}\ \&\ \predica{estuda}{x}$\\


\n Finalmente, temos que:\\

\n \den{S} = \den{CoordP}(\den{NP})\\
...
\n \den{e$'$} = $\lambda F_{\langle e,t\rangle}.\lambda G_{\langle
...
\n \den{e$_{vi}$} = $\lambda F_{\langle e,t\rangle}.\lambda
...
\n \den{e$_{vt}$} = $\lambda F_{\langle e,\langle
	e,t\rangle\rangle}.\lambda G_{\langle e,\langle
...
estas novas funções a extensão \den{e$_{vi}$} e obteremos o mesmo
...
a partir de \den{e$_{vt}$} e \den{e$_{vi}$}. O que fizemos acima foi 
...
Considere a senten\-ça (\ref{neg}) abaixo, à qual atribuiremos a
...
\n \den{não} = $\lambda F_{\langle e,t\rangle }.\ \lambda x_{e}.\
F(\textit{x}) = 0$\\
...
\n \den{VP$_{1}$} = \den{V} = \den{trabalha} = $\lambda x_{e}.\ 
\predica{trabalha}{x}$\\
...
\n \den{NEG} = \den{não} = $\lambda F_{\langle e,t\rangle}.\
\lambda x_{e}.\ \neg F'(x)$\\

\n \den{VP$_{2}$} = \den{NEG}(\den{VP$_{1}$})

\n \den{VP$_{2}$} = ($\lambda F_{\langle e,t\rangle}.\ \lambda x_{e}.\
\neg F'(x)$)(\den{VP$_{1}$}) \\
...
\n \den{NP} = \den{N} = \den{João} = \textit{joão}\\

\n \den{S} = \den{VP$_{2}$}(\den{NP})
...
\n \den{não$_{\langle t,t\rangle}$} = $(\lambda p_{t}.\ p=0)$\\
...
\n \den{não está chovendo} = \den{não}(\den{está chovendo})\\
