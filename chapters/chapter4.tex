...
\n \den{[$_{\text{DP}}$ o prefeito de Campinas ]} = o indivíduo
...
\n \Tree [ [ [ o ].D \qroof{prefeito de Campinas}.NP ].DP \qroof{é
paulista}.VP ].S
...
\n \den{o} = $\lambda F_{\langle e,t\rangle}.\ \text{o indivíduo
...
\n \den{o} = $\lambda F_{\langle e,t\rangle}.\ \iota x[F'(x)]$\\
...
\den{o}(\den{[$_{\text{NP}}$ prefeito de Campinas ]})
...
\n \den{o} = $\lambda F_{\langle e,t\rangle}:\exists !x \in D_{e}[ F'(x)].\ \iota x[F'(x)]$\\
...
\n \den{o} = $\lambda F_{\langle e,t\rangle}:\exists !x_{e}\ \textit{saliente no contexto, tal que}\ [F'(x)].\ \iota x[F'(x)\ \& $ \textit{x} está saliente no contexto $]$\\
...
\n \den{João da Silva} = JS

\n \den{[o prefeito de Campinas]} = o indivíduo \textit{a}, tal
...
\n\den{Arlette Torres} = \den{Fernanda Montenegro} = \textit{fernanda montenegro}\\
...
\n \den{João} = $\lambda x.\ x\ \text{se chama
\textit{João}}$\\
...
contexto}\ [F'(x)].\ \iota x[F'(x)]$)(\den{[$_{\text{NP}}$ João]})
...
\n \den{S} = \den{[$_{\text{VP}}$ está sorrindo]}([$_{\text{DP}}$ o
João])
...
\n \den{João} = $(\lambda x.\ x=\text{\textit{joão}})$\\

\n Para o DP \textit{O João}, obteríamos o seguinte:\\

\n \den{O João} = o único indivíduo \textit{x}, tal que \textit{x}
= \textit{joão}\\
...
\n \den{ele$_{1}$}\gone{1}{João} = João\\
\den{ele$_{1}$}\gone{1}{Pedro} = Pedro\\
\den{ele$_{2}$}\gone{1}{João} = \textit{indefinido}\\
\den{ele$_{1}$}\gvaz = \textit{indefinido}\\

\n No nível sentencial, temos o seguinte:\\

\n \den{Ele$_{1}$ é italiano}\gone{1}{João} = 1 \textit{sse} João é
italiano\\
\n \den{Ele$_{1}$ é italiano}\gone{1}{Pedro} = 1 \textit{sse} Pedro
é italiano\\
\n \den{Ele$_{1}$ é italiano}\gvaz = \textit{indefinido}\\
...
\n\den{Ele$_{1}$ é tio dele$_{2}$}\gtwo{1}{João}{2}{Pedro} = 1 \textit{sse} João é tio de Pedro\\
\den{Ele$_{1}$ é tio dele$_{2}$}\gone{1}{João} = \textit{indefinido}\\
\den{Ele$_{1}$ é tio dele$_{2}$}\gvaz = \textit{indefinido}\\
...
\n 2. \den{PP}$^{g}$ = \den{DP}$^{g}$ =
\den{ela$_{2}$}$^{g}$ = $g(2)$\hfill (1, NR, \textit{de} é vácua)

\n 3. \den{gosta}$^{g}$ = $\lambda x.\lambda y.\ \predica{gosta}{y,x}$ \hfill (L)

\n 4. \den{V}$^{g}$ = \den{gosta}$^{g}$ = $\lambda x.\lambda
...
\n 7. \den{VP}$^{g}$ = $\lambda y.\ \ \predica{gosta}{y,g(2)}$
...
\n 12. \den{S}$^{g}$ = 1 \textit{sse} $\predica{gosta}{g(1),g(2)}$ \hfill
(11,Conversão-$\lambda$)\\

\n As condições de verdade de (\ref{las}) dependem, portanto, do
valor atribuído a \textit{g}. Por exemplo,\\

\n\den{S}\gtwo{1}{Pedro}{2}{Maria} = 1 \textit{sse} Pedro gosta de Maria\\

\n\den{S}\gtwo{1}{João}{2}{Marta} = 1 \textit{sse} João gosta de Marta\\

\n\den{S}\gvaz = \textit{indefinido}\\
